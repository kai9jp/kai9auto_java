...
public class DatabaseCreate {
    ...
    public void exec(Syori3Param s3p) throws CloneNotSupportedException, IOException, JSONException, SQLException {
        try {
            ...
            if (count != 0) {
                s3p.sr3s.updateError(s3p.sr3, "既に存在するDBなので作成できません。「データベース名=" + databaseName + "」");
                return;
            }
            ...
            s3p.sr3s.updateSuccess(this.getClass().getSimpleName(), 100, s3p, "DB作成に成功しました" + crlf + "データベース名=" + databaseName);
            return;
        } catch (Exception e) {
